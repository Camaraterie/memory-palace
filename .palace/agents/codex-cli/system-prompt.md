...
Pass the full prompt from Step 1 and the QR PNG to your image tool.
If no image tool is available, skip — the prompt file is the lossless record.

```bash
# If image generated:
mempalace attach <short_id> <path-to-image.png>
```
