...
function parseContent(text) {
  ...
  return rawSections.map(section => {
    ...
    // First non-empty line starting with Q: is the question
    ...
    // Parse remaining lines into answer blocks
    while (i < sLines.length) {
      const line = sLines[i]
      ...
      if (line.startsWith('- ')) {
        ...
        answerBlocks.push({ type: 'list', items })
        continue
      }
      ...
      if (line.match(/^  \S/)) {
        ...
        answerBlocks.push({ type: 'code', text: codeLines.map(l => l.replace(/^  /, '')).join('\n') })
        continue
      }
      ...
      if (paraLines.length) {
        answerBlocks.push({ type: 'paragraph', text: paraLines.join(' ') })
      }
    }
      ...
      <div style={{ maxWidth: 760, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <p style={{
            ...
            Common questions about Memory Palace, onboarding, templates, and memory recall.
          </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
          {sections.map((section, si) => (
            <article key={si} style={{
              ...
              {/* Answer blocks */}
              {section.answerBlocks.map((block, bi) => {
        ...
        <div style={{
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--stone-text-dim)' }}>
            Need more help?{' '}
            <a href="/troubleshoot" style={{ color: 'var(--brass)', textDecoration: 'none' }}>Troubleshooting guide</a>
            {' '}&middot;{' '}
